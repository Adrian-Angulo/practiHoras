...
class SupabaseRegistrosRepository {
    ...
    async crear(userId, data) {
        ...
        const { data: row, error } = await this.client
            ...
            .single();
        if (error || !row) {
            throw new app_error_js_1.BadRequestError(`Error al registrar jornada en base de datos: ${error?.message}`);
        }
    ...
    async listar(userId, limite) {
        ...
        if (error) {
            throw new app_error_js_1.BadRequestError(`Error al consultar historial de jornadas: ${error.message}`);
        }
        return (data || []).map((r) => this.mapRowToEntity(r));
    }
    async obtenerPorId(userId, id) {
    ...
    async actualizar(userId, id, data) {
        ...
        const { data: row, error } = await this.client
            ...
            .single();
        if (error || !row) {
            throw new app_error_js_1.BadRequestError(`Error al actualizar jornada: ${error?.message}`);
        }
        return this.mapRowToEntity(row);
    }
    async eliminar(userId, id) {
...
//# sourceMappingURL=supabase-registros.repository.js.map